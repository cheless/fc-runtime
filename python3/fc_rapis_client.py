...
class FcRuntimeClient(object):
    ...
    def wait_for_invocation_unblock(self):
        import requests
        response = requests.get(
            url=f"http://{self.fc_runtime_address}/2020-11-11/runtime/invocation/next",
            timeout=None
        )
        ...
        return RequestAsDict(
            ...
            function_type=int(headers.get("x-fc-function-type", 0)),
            function_timeout=int(headers.get("x-fc-function-timeout", 3)),
            retry_count=int(headers.get("x-fc-retry-count", 1)),
            ...
            instance_id=headers.get("x-fc-instance-id"),
